...
class GPEvaluation:
    ...
    def meanSquaredError(self,func):
        mse = [np.square( int(func(*self.X[i])<=0) - self.y[i] ) for i in self.sample]
        return sum(mse)/len(mse)
    def evaluation(self,individual,compile_func):
        func = compile_func(expr=individual)
        return self.meanSquaredError(func), -1.0/len(individual)
...
gpeval = GPEvaluation(X,y)

# primitives, i.e., building blocks
...
def generatePrimitiveSetTyped(random_generator,ind_size):
    pset = gp.PrimitiveSetTyped("MAIN", [np.float64]*ind_size, np.float64, "F")
    pset.addPrimitive(scalarAdd, [np.float64, np.float64], np.float64)
    pset.addPrimitive(scalarSub, [np.float64, np.float64], np.float64)
    pset.addPrimitive(scalarMul, [np.float64, np.float64], np.float64)
    pset.addPrimitive(scalarDouble, [np.float64],np.float64)
    pset.addPrimitive(scalarHalve, [np.float64],np.float64)
    pset.addPrimitive(mulBy3, [np.float64], np.float64)
    pset.addPrimitive(divBy3, [np.float64], np.float64)
    pset.addPrimitive(scalarNeg, [np.float64],np.float64)
    pset.addEphemeralConstant("epm0",lambda: random_generator.uniform(0,1), np.float64)
    return pset

def createBaseObjects(pset, weights=(-0.60, -0.40)):
    creator.create("FitnessMulti", base.Fitness, weights=weights)
    creator.create("Individual", gp.PrimitiveTree, fitness=creator.FitnessMulti)

def createToolbox(pset, eval_class, min_=3, max_=7, tournsize=8):
    toolbox = base.Toolbox()

    toolbox.register("expr", gp.genGrow, pset=pset, min_=min_, max_=max_)
    ...
    toolbox.register("mutate", mutUniformButNoRoot, expr=toolbox.expr_mut, pset=pset)
    ...
    toolbox.register("evaluate", eval_class.evaluation,compile_func=toolbox.compile)

    return toolbox

def mainGPEASimple(toolbox):
    pool = multiprocessing.Pool(processes=multiprocessing.cpu_count())
    toolbox.register("map", pool.map)
    pop = toolbox.population(n=80000)
    ...
    stats_len = tools.Statistics(key=lambda ind:len(ind))
    stats_len.register("avg", np.mean)
    stats_len.register("std", np.std)
    stats_len.register("min", np.min)
...
parser.add_argument("-s","--seed",help="Seed for random number generation.",type=int)
parser.add_argument("-w","--weight",help="Error rate weight",type=float)
...
pset=generatePrimitiveSetTyped(random_generator=defrng,ind_size=X.shape[1])
createBaseObjects(pset,weights=(-args.weight,-(1.0-args.weight)))
toolbox=createToolbox(pset,gpeval,max_=args.depth)

def check_validation_gp(toolbox,best_ind,validation_x,validation_y,weights):
    res = []
    for individual in best_ind:
        func=toolbox.compile(expr=individual)
        mse = [np.square(int(func(*validation_x[i])<=0)- validation_y[i]) for i in range(len(validation_y))]
        size = (-1.0/len(individual)) * weights[1]
        res.append((individual,(sum(mse)/len(mse))*weights[0]+size ))
    res.sort(key=lambda x:x[1],reverse=False)
    return res

if __name__=="__main__":
    start_time = time.time()
    pop, logbook, hof, mstats = mainGPEASimple(toolbox)
    end_time = time.time()

    twibot20GP = {"logbook":logbook,"hof":[hof[i] for i in range(0,len(hof))],
                         "mstats_last_pop":mstats.compile(pop),"executionTimeInHours":(end_time-start_time)*(1/3600)}
    print(twibot20GP["executionTimeInHours"])
    file_name = "twibot20GP"+"_"+str(args.ind)+".pickle"
    pickling_on = open(file_name,"wb")
    pickle.dump(twibot20GP, pickling_on)
    pickling_on.close()
    ...
    res = check_validation_gp(toolbox,hof,X_de,y_dev,weights=(-args.weight,-(1.0-args.weight))) # evaluating the hall of fame on the validation set
    ...
    # python train.py -i 1 -s 68 -w 0.60 -d 7
